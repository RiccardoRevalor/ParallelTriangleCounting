...
__global__ void EdgeIteratorAlgorithmKernel(
    ...
    //Check if the current thread's index is within the valid range for this chunk
...
int main(int argc, char **argv) {
    ...
    int desiredLaunches = std::stoi(argv[3]);
    ...
    int chunkSize = (numEdges + desiredLaunches - 1) / desiredLaunches; //size of each chunk


    //start kernel function
    auto startTime = chrono::high_resolution_clock::now();
    for (int current_start_edge_idx = 0; current_start_edge_idx < numEdges; current_start_edge_idx += chunkSize) {
        int current_end_edge_idx = min(current_start_edge_idx + chunkSize, numEdges);
        int num_edges_in_chunk = current_end_edge_idx - current_start_edge_idx;

        if (num_edges_in_chunk == 0) continue; // Skip empty chunks

        // Calculate gridSize for the current chunk
        // Each thread processes one edge within the chunk.
        // The total number of threads needed for this chunk is num_edges_in_chunk.
        int gridSize = (num_edges_in_chunk + blockSize - 1) / blockSize;

        EdgeIteratorAlgorithmKernel<<<gridSize, blockSize>>>(
            num_edges_in_chunk, // Pass the number of edges in this specific chunk
            d_adjacencyList_rowPtr,
            d_adjacencyList_colIdx,
            d_edgeVector,
            d_ranks,
            d_countTriangles,
            current_start_edge_idx, // Pass the start index of the current chunk
            current_end_edge_idx    // Pass the end index of the current chunk
        );
        CUDA_CHECK(cudaGetLastError()); // Check for errors after each kernel launch
    }
    ...
    string outputFileName("../../cross_validation_output/cuda_edge_it_v1_1/" + input + "_" + gpuModel + ".csv");
    ...
    if (crossValidationFile.tellp() == 0) { // Check position
        crossValidationFile << "BLOCK_SIZE,DESIRED_LAUNCHES_KERNEL,GPU_MODEL,TOTAL_DURATION_US,TRIANGLES\n";
    }
