...
__global__ void EdgeIteratorDynamicQueueKernel(
    int numEdges,
    ...
    int* d_workQueueIndex //work queue pointer
) {
    
    while (true) {
        // 1. PRELEVA ATOMICAMENTE UN INDICE DALLA CODA
        // Ogni thread incrementa il contatore globale e ottiene un indice di arco univoco.
        // atomicAdd garantisce che due thread non prenderanno mai lo stesso indice.
        int edge_idx = atomicAdd(d_workQueueIndex, 1);

        // 2. CONDIZIONE DI USCITA
        // Se l'indice supera il numero totale di archi, significa che tutto il lavoro è stato fatto.
        // Il thread può quindi uscire dal ciclo e terminare.
        if (edge_idx >= numEdges) {
            break;
        }

        // 3. PROCESSO DELL'ARCO
...
int main(int argc, char **argv) {
    ...
    EdgeIteratorDynamicQueueKernel<<<gridSize, blockSize>>>(
        numEdges,
    ...
    string outputFileName("../../cross_validation_output/cuda_edge_it_v3/" + input + "_" + gpuModel + ".csv"); // v3_dynamic
